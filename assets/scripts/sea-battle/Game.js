...
SeaBattle.Game.prototype.fire = function(playerId, x, y) {
  ...
  if (this._state.currentPlayerId === playerId && this._state.status === SeaBattle.Game.StatusEnum.Gaming) {
    ...
    if (typeof targetFireSchema[x][y] !== 'number') {
      if (targetShipsSchema[x] && targetShipsSchema[x][y] && targetShipsSchema[x][y].busy === true) {
        targetFireSchema[x][y] = 1;
        result = 'hit';
        var isShipsDestroyed = this.isShipsDestroyed(targetShipsSchema, targetFireSchema);
        if (isShipsDestroyed) {
...
SeaBattle.Game.prototype.isShipsDestroyed = function(shipsSchema, fireSchema) {
  var result = true;

  for(var i = 0; i < this._fieldSize; i++) {
    for (var j = 0; j < this._fieldSize; j++) {
      if (shipsSchema[i] && shipsSchema[i][j] && shipsSchema[i][j].busy === true &&
